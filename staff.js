(() => {
  ...
  const PUBLIC_SHEET='Учителя_сайт', BASE_TEACHERS='Учителя', SCHEDULE=config.googleSheets?.sheets?.schedule||'Расписание';
  ...
  function publicCellsForDate(){if(!state.publicRows.length)return[];const h=state.publicRows[0],d=state.publicRows.slice(1);const ix={date:headerIndex(h,['Дата'],0),teacher:headerIndex(h,['Учитель'],1),lesson:headerIndex(h,['Урок'],2),cls:headerIndex(h,['Класс'],3),subject:headerIndex(h,['Предмет'],4),room:headerIndex(h,['Кабинет'],5),status:headerIndex(h,['Статус'],6),oldClass:headerIndex(h,['Было_класс'],7),oldSubject:headerIndex(h,['Было_предмет'],8),oldRoom:headerIndex(h,['Было_кабинет'],9),note:headerIndex(h,['Примечание'],10)};return d.filter(r=>normalizeDate(r[ix.date])===state.date).map(r=>({teacher:String(r[ix.teacher]||'').trim(),lesson:Number(r[ix.lesson])||0,className:normalizeClass(r[ix.cls]),subject:fullSubject(String(r[ix.subject]||'').trim()),room:String(r[ix.room]||'').trim(),status:String(r[ix.status]||'normal').trim()||'normal',oldClass:normalizeClass(r[ix.oldClass]),oldSubject:fullSubject(String(r[ix.oldSubject]||'').trim()),oldRoom:String(r[ix.oldRoom]||'').trim(),note:String(r[ix.note]||'').trim()})).filter(x=>x.teacher&&x.lesson>=1&&x.lesson<=12)}
  ...
  function syncTeacherSelects(){['#teacher-select','#staff-my-teacher'].forEach(sel=>{const e=$(sel);if(!e)return;e.innerHTML=state.teachers.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('');e.value=state.selectedTeacher})}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('#staff-search').addEventListener('input',e=>{state.search=e.target.value;renderGrid()});
    ...
    $('#teacher-select').addEventListener('change',e=>{saveTeacher(e.target.value);renderMobile()});
